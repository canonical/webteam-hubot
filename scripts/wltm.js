...
async function googleSpreadsheetHandler(acronym) {
  ...
  var responses = rows.filter(
    (a) => a.Acronym && a.Acronym.toUpperCase().trim() === acronym
  );
...
module.exports = function (robot) {
  robot.router.post("/hubot/wltm", async function (req, res) {
    ...
    const result =
      `Would Like to Meet helps you set up GoogleMeet 'coffee-chats' with a random Canonicaller. When you join you will receive invitations to 25 minute meets, up to 6 times a year. help for more.`;
